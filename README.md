...
Linux 服务端口限速工具。默认每个端口上传和下载各 **15 Mbit/s**，两方向独立；可为每个端口设置不同速率，TCP/UDP 可选。`pf` 打开中文菜单。适用于 Linux 服务器上的本机服务端口。
...
- 脚本仅管理 `tc` ingress/egress 优先级 42424。若网卡已有使用该优先级的过滤器，请勿安装；`clsact` 本身不会被卸载。已有的其他限速策略仍可能影响最终速度。
